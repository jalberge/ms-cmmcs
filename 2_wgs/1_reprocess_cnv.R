...
ccf.df <- rbindlist(ccf.dlist, idcol = "Pair") %>% 
  ...
  inner_join(clinicaldata, by=c("Pair"="entity:sample_id"))
...
# Could also use near ~2 exact ploidy - doesn't change heatmap
# simplified compared to chip's code. ask him.
...
CNV.bed.clean %>% 
  ...
  write_tsv("data/March9_reprocessed.cnv.prod5.seg")

# 
# # Clean seg file ----------------------------------------------------------
# 
# seg.file <- read_tsv("data/Feb7_reprocessed.cnv.prod5.seg")
# seg.file %>% 
#   filter(`entity:sample_id` %in% clinicaldata$`entity:sample_id`) %>% 
#   write_tsv("data/Feb7_reprocessed.cnv.prod5.seg")
