...
# sv <- read_tsv("../data/CMMCsPaper2022_ConsensusSV_filtered_results.txt")
# sv <- read_tsv("../data/Prod5.txt")
sv <- read_tsv("../data/Prod6.txt")
...
Pairs <- Pairs[Pairs %in% sv$individual]
Pairs <- Pairs[Pairs %in% c("CTF047_BMPCs", "CTF047_CMMCs_75k")]


# Write IGV bash script for cohort structural variant ---------------------

script.file <- "../igvsessions/March3_BS_igv_svs.script"
