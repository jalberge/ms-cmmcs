...
ldfr.fixed.protein.change.clinical.annot <- ldfr.fixed.protein.change %>%
  ...
  filter(Timepoint %in% c("T0", "T1")) %>% # special hack for CTF019 to be found. + T2 is myeloma.
...
long.mut <- tiny.flat.matched.maf %>%
  ...
  # geom_point(aes(y=preDP_ccf_mean_T1), fill=longitudinal.pal["T1"], position=position_nudge(x=0.2), size=1, shape=2, color="#3B3B3B") +
  ...
  # geom_point(aes(y=preDP_ccf_mean_T0), fill=longitudinal.pal["T0"], position=position_nudge(x=-0.2), size=1, shape=2, color="#3B3B3B") +
  ...
  # scale_color_manual(values=longitudinal.pal) +
  # scale_fill_manual(values=longitudinal.pal) +
...
#   filter((clust_ccf_mean_T1<.9 | clust_ccf_mean_T0<.9)) %>% # non clonal mutations
...
#   pivot_longer(cols=c(clust_ccf_mean_T1, clust_ccf_mean_T0), names_to = "tissue", values_to = "Mean_CCF") %>%
#   mutate(tissue=str_extract(tissue, "(T0|T1)")) %>%
